...
export interface StoryBook extends LibraryGroup { synopsis: string; direction: string; collectionIds: string[] }
export interface WorldLibrary {
  revision: number; albums: LibraryGroup[]; photos: AlbumPhoto[]; locations: LocationSetting[]; storyBooks: StoryBook[]
  boardBooks: Record<string, string>
}
export const emptyWorldLibrary = (): WorldLibrary => ({ revision: 0, albums: [{ id: 'default', name: 'My images', description: '', updatedAt: 0 }], photos: [], locations: [], storyBooks: [], boardBooks: {} })
...
export const WORLD_LIBRARY_SCHEMA = obj({
  ...
  storyBooks: array(obj({ ...group, synopsis: str(), direction: str(), collectionIds: array(id, 100) })),
  boardBooks: { type: 'object', maxProperties: 1000, additionalProperties: id },
})
...
export function validateWorldLibrary(input: unknown): WorldLibrary {
  ...
  const { folders: _folders, boardFolders: _boardFolders, ...current } = legacy
  const result = validator.safeParse({ ...current, storyBooks: current.storyBooks ?? _folders ?? [], boardBooks: current.boardBooks ?? _boardFolders ?? {} })
  if (!result.success) throw new Error(`Invalid library: ${result.error.issues[0]?.message}`)
  const library = result.data as WorldLibrary
  for (const group of [library.albums, library.photos, library.locations, library.storyBooks, ...library.locations.map((l) => l.conditions)]) {
  ...
  if (Object.keys(library.boardBooks).some((key) => !/^[a-zA-Z0-9_-]{1,100}$/.test(key))) throw new Error('Invalid storyboard ID')
  ...
  if (Object.values(library.boardBooks).some((id) => !library.storyBooks.some((book) => book.id === id))) throw new Error('Story book not found')
