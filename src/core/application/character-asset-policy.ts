...
export const CHARACTER_REPLACE_ASSET_TOOL_GUIDANCE = `Replace one target-owned genuine RGBA PNG layer after inspect_character_contract and complete metadataStatus. Every lane uses the same inspect, preflight, replace and visual-review workflow; lane-specific ownership, layers and preparation come from assetPolicy.lanes. AOZU derives coverageContract from the lane and outfit.slot. Outfit, hair and headwear submissions require the same preflightPoints used to inspect the candidate. Use rebaseDerivedAssets:true only for compatible small base corrections. Accepted means stored, not visually approved. ${CHARACTER_NAVIGATION_GUIDANCE}`
