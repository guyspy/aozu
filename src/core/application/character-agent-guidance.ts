...
export const CHARACTER_BACKGROUND_GUIDANCE = 'AOZU accepts only genuine RGBA PNG layers. For newly generated art, first prepare a permitted background-removal tool available in the environment (for example Apple Vision foreground masking on macOS, an image editor, or a local image-processing library). Generate on one flat high-contrast color absent from the subject, then remove it without cropping or reframing. Alpha is file data, not a visual style. Never generate a checkerboard. If a generated image contains a painted grid, stop repeating transparency prompts and use the prepared background-removal method. An uploader transfers bytes; it cannot remove a background. Verify real alpha and inspect edges on light and dark backgrounds. For user-supplied finished art, preserve the requested pixels, alpha, glow and edge treatment; do not regenerate or remove them to satisfy a default recipe. Check resized output against the source before submitting; dimensions and hashes do not prove visual fidelity. AOZU never removes backgrounds.'
