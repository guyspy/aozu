...
export const CHARACTER_A_POSE_GUIDANCE = 'The first Appearance body establishes a front A-pose: upright torso, neutral face, arms angled down and away from the torso, relaxed visible hands, stable feet, and the complete head-to-feet silhouette inside the canvas. Use neutral technical basewear made only for wardrobe registration: a plain, opaque, close-fitting sleeveless athletic top and fitted mid-thigh shorts that preserve ordinary coverage of the chest, pelvis, and buttocks while leaving the shoulders, arms, legs, and body contours unobstructed. Do not style the base as underwear, lingerie, swimwear, nudity, or suggestive clothing; avoid sheer fabric, cleavage emphasis, loose fabric, footwear, jewelry, and accessories. This is the AOZU authoring convention, not a claim of skeleton compatibility. Preserve this pose and registration in derived outfits, hair, headwear, and expressions. Existing artwork is not automatically verified A-pose; review it visually before proposing a baseline change, which can make derived layers stale.'
