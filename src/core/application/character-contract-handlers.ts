  ...
  async function inspectModelSheetContract(rawInput: unknown) {
    ...
    return { status: 'ok', data: {
      ...
      productionBrief: [
        ...
        'Build the model sheet one image at a time: capture the current Appearance into front, then generate three-quarter, side and back as separate files and referenceIds. Re-inspect after each save for the next revision. Do not place a multi-view collage in a turnaround slot or create a second mandatory A-pose. Raised-arm images use separate supplemental IDs with kind:structure. Save as captures the new look’s front; add new leaves references empty until you add them or edit the composition.',
