...
export const CHARACTER_BACKBONE_SOURCES = [
  ...
  source(
    ...
    envelope('Procedure', 'update-character-model-sheet', {
      ...
      description: `${PNG_WEBMCP_TRANSFER_GUIDANCE} Edit the active Appearance’s model sheet after inspect_character_contract with scope:model-sheet. One image per referenceId: capture the current combination as front with fromAppearance:true, then generate three-quarter, side and back as separate files. Save and visually review each before re-inspecting for the next revision. Raised-arm poses use a separate supplemental referenceId with kind:structure; never put a multi-view collage in a turnaround slot. References belong to modelSheet.appearanceId; switch saved sets through set_character_variant_selection, then re-inspect. Composition edits autosave into the current Appearance and synchronize linked front images. Other references may have needsReview:true; visually compare against the updated Appearance, then replace them or set needsReview:false. This is a consistency flag, not user approval. Optional shared height is edited through update_character_profile. Other IDs create supplemental references with label and kind. view is a compatibility alias for a default slot. Optional viewpoint and pose describe that reference. PNGs may be opaque and keep original dimensions up to 4096 × 4096 / 5 MiB. For replacement supply filename, exact expectedAssetSha256 (null for empty), dataUrl, and optional dataSha256. fromAppearance:true takes no image input or filename. sourceSha256 identifies the source image used for generated art. remove:true deletes only this reference and requires its exact hash. All edits use expectedRevision. Replacement clears old guides and source hash unless a new source is supplied. Notes and guides need existing art; guides are original-image y fractions from the top with head above feet, excluding hats, raised arms and props. Never infer height from pixels; null clears guides. Omitted fields remain unchanged. accepted:true means stored, not visually verified or user-approved. Follow visualReview on the exact reference. ${CHARACTER_NAVIGATION_GUIDANCE}`,
      input: objectSchema({
        characterId: { type: 'string', minLength: 1 },
        expectedRevision: { type: 'integer', minimum: 0 },
