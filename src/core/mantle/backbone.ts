...
const workspaceNavigationSchema = objectSchema({
  resource: { enum: ['home', 'collections', 'collection', 'locations', 'location', 'albums', 'album', 'photo', 'storyboards', 'folder', 'storyboard', 'character'] },
  id: { type: 'string', minLength: 1, maxLength: 100 },
  view: { enum: ['characters', 'profile', 'locations', 'expressions', 'outfits', 'props', 'model-sheet', 'storyboard', 'details'] },
  itemId: { type: 'string', minLength: 1, maxLength: 100 },
}, ['resource'])

const libraryUpdateSchema = objectSchema({
  resource: { enum: ['collection', 'character', 'album', 'photo', 'location', 'condition', 'reference', 'folder', 'storyboard-folder'] },
  ...
  folderId: { oneOf: [{ type: 'string', minLength: 1, maxLength: 100 }, { type: 'null' }] },
}, ['resource', 'action'])
...
const ALL_BACKBONE_SOURCES = [
  ...
  ...[
    ...
    { name: 'navigate-workspace', title: 'Navigate Workspace', description: 'Open an exact AOZU resource without guessing a route. resource is home, collections, collection, locations, location, albums, album, photo, storyboards, folder, storyboard, or character. Supply id for one resource; character view may be expressions, outfits, props, profile, or model-sheet and itemId opens a variant/reference. Collection view may be characters, profile, or locations; storyboard view may be storyboard or details. AOZU applies the returned navigation in this tab. Re-run inspect_workspace after rendering.', input: { ...workspaceNavigationSchema, readOnly: true } },
    { name: 'update-library', title: 'Update Library', description: 'Create, update, delete, or move AOZU library records with one revision-checked command. Resources: collection; character (move between collections or delete); album; photo; location; condition; reference (explicitly reuse a finished Album photo as Location inspiration/design); folder; storyboard-folder. Use IDs and expectedRevision from inspect_workspace. Omitted update fields stay unchanged. Deleting an Album moves its photos to My images; deleting a Location moves its children to its parent; deleting a Folder leaves its storyboards unfiled. This does not edit storyboard frames or character artwork. AOZU itself handles effects.navigation.', input: libraryUpdateSchema },
    { name: 'export-library', title: 'Export Library Resource', description: 'Download one resource without returning a huge base64 payload. resource may be library (complete backup), world (Albums, Locations and Folders), character, storyboard, or photo. Character/storyboard require id and exact expectedRevision. Photo requires id. Returns the filename and size after starting the browser download.', input: { ...objectSchema({ resource: { enum: ['library', 'world', 'character', 'storyboard', 'photo'] }, id: { type: 'string', minLength: 1, maxLength: 100 }, expectedRevision: { type: 'integer', minimum: 0 } }, ['resource']), readOnly: true } },
  ...
  source(
    ...
    envelope('Procedure', 'inspect-workspace', {
      ...
      description: `Start here and call again after user navigation or tool mutations: context is a snapshot, not a live subscription. intent:world starts the required world-authoring sequence: first choose and inspect a Collection's backstory, Characters, Locations and Conditions, then create setting art or a composed Album photo. resource/id can inspect an exact collection, album, photo, location or folder without navigating; images requests up to five Album photos or direct Location setting images with original data URLs. includeSnapshot:true returns the current Character composite or exact open/requested Photo, Location design, or model-sheet reference. Actually view returned image data before visual feedback. For storyboard originals use inspect_storyboard. A snapshot never navigates, saves or changes selections.`,
      input: {
        ...objectSchema({
          ...
          resource: { enum: ['collection', 'album', 'photo', 'location', 'folder'] },
