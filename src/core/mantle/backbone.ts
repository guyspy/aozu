...
const ALL_BACKBONE_SOURCES = [
  ...
  source(
    ...
    envelope('Procedure', 'replace-character-asset', {
      ...
      description: `${PNG_WEBMCP_TRANSFER_GUIDANCE} Install one target-owned Character layer after inspect_character_contract with scope:appearance. ${CHARACTER_A_POSE_GUIDANCE} Submission contract: body is the complete base body; expression is one complete whole head; outfit is garment pixels only; hair, headwear, and prop contain only their own pixels. A dressed character may be used temporarily to create an outfit, but never submit that intermediate or any body pixels to a Wardrobe slot. AOZU mechanically rejects overlays that resemble a complete character. Follow backgroundPreparation: solid-color generation, removal with a permitted environment tool, then alpha/edge verification. This is a true replacement without preserving old pixels. Registered outfit, hair, and headwear overlays may split into front and back. When replacing an existing body with registered layers, explicitly set rebaseDerivedAssets: true only if the new body preserves the same canvas registration, pose, and silhouette; this keeps existing pixels and marks them current without altering them. Set false to intentionally leave those layers stale. Opaque input and body art without a fully opaque subject core are rejected; AOZU never removes backgrounds. Rejected or stale input does not mutate or navigate. Supply one complete dataUrl and optional dataSha256. Do not submit a URL, attachment reference, truncated text, or JPEG/WebP bytes relabeled as PNG. Submit exact ${CHARACTER_RIG.canvas.width}×${CHARACTER_RIG.canvas.height} RGBA or explicitly request the inspected normalization. After variant acceptance, follow the returned alignment.visualReview through all four browser modes before the next asset; review the canonical body in its regular Composite preview. ${CHARACTER_NAVIGATION_GUIDANCE}`,
      input: objectSchema({
        characterId: { type: 'string', minLength: 1 },
        group: { enum: CHARACTER_VARIANT_GROUPS },
        variantId: { type: 'string', pattern: '^[a-z0-9][a-z0-9_-]{0,39}$' },
