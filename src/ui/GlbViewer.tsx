...
export const GlbViewer = memo(function GlbViewer({ label, className, controls = true }: { label: string; className?: string; controls?: boolean }) {
  ...
  useEffect(() => {
    ...
    let destroy = () => {}
    let unsubscribe = () => {}
    const mount = () => void import('@/adapters/browser/glb-character-viewer').then(async ({ mountGlbCharacterViewer }) => {
    ...
    // Closed overflow lists and offscreen cards must not allocate WebGL contexts.
    const observer = new IntersectionObserver(entries => {
      if (entries.some(entry => entry.isIntersecting)) { observer.disconnect(); mount() }
    })
    if (host.current) observer.observe(host.current)
    return () => { disposed = true; observer.disconnect(); unsubscribe(); destroy() }
  }, [controls])
  return (
