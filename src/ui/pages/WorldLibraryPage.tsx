import { WorkspaceSheet, Workspace, WorkspaceAdd, WorkspaceAddCard, WorkspaceCard, WorkspaceSurface, WorkspaceTabs, WorkspaceToolbar, WorkspaceScroll } from '@/ui/Workspace'
...
export function WorldLibraryPage({ service, library, collections, actions }: { service: WorldLibraryService; library: WorldLibrary; collections: CharacterCollection[]; actions: ReactNode }) {
  ...
  return <Workspace header={<>{collectionId && <WorkspaceTabs label={text('collection')} active="locations"
    ...
    {(place || photo) && <WorkspaceToolbar><div className="world-actions ml-auto">
      <Button variant="outline" onClick={() => begin(place ? 'location' : 'photo', place ?? photo)}><PencilIcon />{text('edit')}</Button>
    </div></WorkspaceToolbar>}</>} className="world-workspace" data-workspace-view={collectionId ? 'locations' : 'albums'} data-collection-id={collectionId} data-location-id={locationId} data-album-id={albumId} data-photo-id={photoId} data-has-uncommitted-input={Boolean(editing || reference)}>
