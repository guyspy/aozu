...
export function StoryboardPage({ service }: { service: StoryboardService }) {
  ...
  return <main className="story-workspace" data-workspace-view={boardId ? 'storyboard' : 'storyboards'} data-board-id={boardId} data-board-revision={board?.revision} data-frame-id={frameId || undefined} data-panel={frameId ? 'frame' : undefined} data-has-uncommitted-input={dirty} data-compared-image-ids={compare.join(',')} data-candidate-id={compare.length === 1 ? compare[0] : frame?.selected ?? undefined}>
    <Link className="text-sm text-muted-foreground" to={boardId ? '/storyboards' : '/collections'}>← {text(boardId ? 'title' : 'home')}</Link>
