...
export function StoryboardSettingPicker({ application, world, collections, characters, collectionIds, frame, disabled, pin }: { application: Application; world: WorldLibrary; collections: CharacterCollection[]; characters: CharacterLibraryItem[]; collectionIds?: string[]; frame: BoardFrame; disabled: boolean; pin(command: BoardCommand, blob?: Blob): Promise<unknown> }) {
  ...
  const allowed = new Set(collectionIds?.length ? collectionIds : collections.map(({ id }) => id))
  const choices = kind === 'location' ? world.locations.filter((item) => allowed.has(item.collectionId)) : kind === 'character' ? characters.filter((item) => collections.some((collection) => allowed.has(collection.id) && collection.characterIds.includes(item.id))) : world.photos
  const run = async () => {
