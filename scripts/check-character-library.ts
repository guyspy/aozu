...
await assert.rejects(readCharacterLibraryZip(await mutateManifest((value) => value.entries[0].data.selected.items = [...value.entries[0].data.selected.items.filter((item) => item.group !== 'prop'), { group: 'prop', id: 'missing' }]), inspect), /selection/)
await assert.rejects(readCharacterLibraryZip(await mutateManifest((value) => value.entries[0].data.selected.items = [...value.entries[0].data.selected.items.filter((item) => item.group !== 'prop'), { group: 'prop', id: 'prop-1' }, { group: 'prop', id: 'prop-1' }]), inspect), /schema|selection/)
await assert.rejects(readCharacterLibraryZip(await mutateManifest((value) => value.entries[0].data.selected.items = [...value.entries[0].data.selected.items.filter((item) => item.group !== 'outfit'), { group: 'outfit', id: 'missing' }]), inspect), /selection/)
