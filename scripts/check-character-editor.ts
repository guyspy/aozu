...
await preparedEditor.dispatch((draft) => ({ ...draft, selected: { smartOrder: true, items: [...draft.selected.items.filter((item) => item.group !== 'expression'), { group: 'expression', id: 'happy' }] } }))
assert.equal(preparedEditor.store.getState().saveStatus, 'failed')
assert.equal(rows.get(namedDraft.id)!.character.selected.items.find((item) => item.group === 'expression')?.id, undefined)
...
assert.equal(rows.get(namedDraft.id)!.character.appearances![1]!.selected.items.find((item) => item.group === 'expression')?.id, 'happy')
