...
if (new URLSearchParams(location.search).has('responsive')) {
  ...
  check(button('Download complete backup') && document.querySelector('.library-tree-transfer input[type="file"]'), 'Library tree owns complete import and export')
  ...
  button('Close').click()
  await ready(() => !document.querySelector('.library-explorer'))
  check([...document.querySelectorAll('.workspace-tabs .workspace-tab')].map((el) => el.textContent.trim()).join('|') === 'Collections|Albums|Storyboards', 'Landing must offer the three library areas')
  ...
  button('Conditions').click()
  await ready(() => button('Saved conditions'))
  button('Saved conditions').dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, button: 0, isPrimary: true }))
  await ready(() => menuItem('Add condition'))
  menuItem('Add condition').click()
  await ready(() => document.querySelector('.world-form input[name="name"]'))
  enter(document.querySelector('.world-form input[name="name"]'), 'Night')
  document.querySelector('.world-form').requestSubmit()
  await ready(() => !document.querySelector('[role="dialog"]') && button('Saved conditions')?.textContent.includes('Night'))
  ...
  await ready(() => button('Generation details'))
  ...
  await ready(() => button('Add empty frame'))
  check(document.querySelector('.app-breadcrumb [aria-current="page"]').textContent.length > 0, 'Named current photo or storyboard crumb')
  button('Add empty frame').click()
  ...
  check(scopedActions.includes('Move to story book'), 'Storyboard actions include book assignment')
  check(!document.querySelector('.story-document button[aria-label="Story book"]'), 'Storyboard paper omits the book selector')
  ...
  await ready(() => button('Create story book'))
  ...
  result.textContent = 'PASS: home, collections, nested locations, conditions, album references, album-to-board, story books, parent navigation and logo home'
