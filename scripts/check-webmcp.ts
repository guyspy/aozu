...
assert.match(registered.get('replace_character_asset')!.description, /First inspect_workspace, then inspect_character_contract/)
assert.match(registered.get('replace_character_asset')!.description, /移除此圖像的背景。保持所有前景主體不變且完整，邊緣乾淨平滑。將背景設為透明。/)
assert.match(registered.get('replace_character_asset')!.description, /hanging hair.*in back/)
...
assert.match(registered.get('replace_character_asset')!.description, /same preflightPoints/)
