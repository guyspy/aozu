...
assert.deepEqual(fresh.selected, { outfits: {}, props: [] })
...
const another = changeCharacterAppearance({ ...first, selected: { outfits: { top: 'top-1' }, expression: 'happy', props: ['prop-1'] } }, { action: 'save-as', id: 'formal', label: 'Formal' })
...
assert.equal(sameCharacterSelection({ outfits: {}, props: ['a', 'b'] }, { outfits: {}, props: ['b', 'a'] }), false, 'prop order is part of the combination')
...
assert.throws(() => validateCharacterAppearances({ ...first, appearances: [{ ...first.appearances![0], selected: { outfits: {}, props: ['missing'] } }] }), /missing/)
...
await editor.dispatch((current) => ({ ...current, selected: { ...current.selected, expression: 'happy' } }))
assert.equal(editor.store.getState().character?.appearances?.[0].selected.expression, 'happy', 'Editing autosaves the active named Appearance')
...
assert.equal(editor.store.getState().character?.selected.expression, undefined)
assert.equal(editor.store.getState().character?.appearances?.[0].selected.expression, undefined)
assert.deepEqual(editor.store.getState().character?.appearances?.[1], withSide.appearances?.[1], 'Undo preserves another Appearance')
await editor.redo()
assert.equal(editor.store.getState().character?.appearances?.[0].selected.expression, 'happy')
