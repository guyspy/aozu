...
assert.deepEqual(fresh.selected, { smartOrder: true, items: [] })
...
const another = changeCharacterAppearance({ ...first, selected: { smartOrder: true, items: [{ group: 'outfit', id: 'top-1' }, { group: 'expression', id: 'happy' }, { group: 'prop', id: 'prop-1' }] } }, { action: 'save-as', id: 'formal', label: 'Formal' })
...
assert.equal(sameCharacterSelection({ smartOrder: true, items: [{ group: 'prop', id: 'a' }, { group: 'prop', id: 'b' }] }, { smartOrder: true, items: [{ group: 'prop', id: 'b' }, { group: 'prop', id: 'a' }] }), false, 'prop order is part of the combination')
...
assert.throws(() => validateCharacterAppearances({ ...first, appearances: [{ ...first.appearances![0], selected: { smartOrder: true, items: [{ group: 'prop', id: 'missing' }] } }] }), /missing/)
...
assert.equal(await editor.undo(), false, 'Switching Appearance is navigation')
await editor.dispatch((current) => ({ ...current, selected: { smartOrder: true, items: [...current.selected.items.filter((item) => item.group !== 'expression'), { group: 'expression', id: 'happy' }] } }))
assert.equal(editor.store.getState().character?.appearances?.[0].selected.items.find((item) => item.group === 'expression')?.id, 'happy', 'Editing autosaves the active named Appearance')
await editor.dispatch((current) => ({ ...current, name: 'Profile survives' }))
await editor.undo()
assert.equal(editor.store.getState().character?.name, 'Profile survives')
assert.equal(editor.store.getState().character?.selected.items.find((item) => item.group === 'expression')?.id, undefined)
assert.equal(editor.store.getState().character?.appearances?.[0].selected.items.find((item) => item.group === 'expression')?.id, undefined)
assert.deepEqual(editor.store.getState().character?.appearances?.[1], withSide.appearances?.[1], 'Undo preserves another Appearance')
await editor.redo()
assert.equal(editor.store.getState().character?.appearances?.[0].selected.items.find((item) => item.group === 'expression')?.id, 'happy')
